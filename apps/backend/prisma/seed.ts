...
import { RULES_2026, COMPETITION_A, COMPETITION_B_NATIONAL, COMPETITION_B_PROVINCIAL, COMPETITION_B_MUNICIPAL, CERT_LANG_ENTRIES } from './rules-2026';
...
async function main() {
  ...
  for (const r of RULES_2026) {
    await prisma.ruleItem.upsert({
      ...
      update: {
        ...
        detailSchema: r.detailSchema as any,
        defaultScore: r.defaultScore ?? null,
        levelScoreMap: (r.levelScoreMap ?? null) as any,
        caps: (r.caps ?? {}) as any,
  ...
  console.log(`✔ 规则字典: ${RULES_2026.length} 项（品德/学业/文体）`);

  // 3. 白名单：竞赛目录（2026 认定结果）+ 语言证书。同 (type,name,year) 幂等。
  const year = 2026;
  const compEntries: { type: string; name: string; level: string }[] = [
    ...COMPETITION_A.map((name) => ({ type: 'COMP_A', name, level: 'NATIONAL' })),
    ...COMPETITION_B_NATIONAL.map((name) => ({ type: 'COMP_B_NATIONAL', name, level: 'NATIONAL' })),
    ...COMPETITION_B_PROVINCIAL.map((name) => ({ type: 'COMP_B_PROVINCIAL', name, level: 'PROVINCIAL' })),
    ...COMPETITION_B_MUNICIPAL.map((name) => ({ type: 'COMP_B_MUNICIPAL', name, level: 'MUNICIPAL' })),
  ];
  ...
  console.log(`✔ 竞赛白名单: 目录 ${compEntries.length} 项（A类${COMPETITION_A.length}/国家级${COMPETITION_B_NATIONAL.length}/省级${COMPETITION_B_PROVINCIAL.length}/市级${COMPETITION_B_MUNICIPAL.length}），新增 ${compAdded}`);
  ...
  let certAdded = 0;
  for (const c of CERT_LANG_ENTRIES) {
    if (certSeen.has(c.name)) continue;
    await prisma.whitelistEntry.create({ data: { type: 'CERT_LANG', name: c.name, year, extra: c.extra as any } });
    certSeen.add(c.name);
    certAdded++;
  }
  console.log(`✔ 语言证书白名单: ${CERT_LANG_ENTRIES.length} 项，新增 ${certAdded}`);
